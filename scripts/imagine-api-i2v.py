#!/usr/bin/env python3
"""Image-to-video via official xAI API.

Auth: XAI_API_KEY → $HOME/.grok/auth.json → /root/.grok/auth.json
(see scripts/imagine_auth.py). Downloads video.url immediately (vidgen.x.ai).
...
def req(method: str, url: str, token: str, body=None, timeout=60):
...
def main() -> None:
    ...
    for i in range(48):
        time.sleep(5)
        rec = req("GET", f"{API}/videos/{rid}", token)
